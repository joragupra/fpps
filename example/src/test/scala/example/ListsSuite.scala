...
class ListsSuite extends FunSuite {
  ...
  test("sum with negative numbers") {
    assert(sum(List(4,-3,2))===3)
  }
  
  test("sum with zero value numbers") {
    assert(sum(List(2,0,4))===6)
  }
  
  test("max of an emty list") {
  ...
  test("max with negative numbers") {
    assert(max(List(3,-10,2))===3)
  }
  
  test("max with repeted numbers") {
    assert(max(List(3,3,10))===10)
  }
}
